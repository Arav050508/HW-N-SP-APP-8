    ...
    function showTaskDetails(event) {
        ...
        deleteTaskButton.onclick = async function() {
            ...
            if (user) {
                try {
                    const userId = user.uid;
                    const taskRef = db.collection("tasks").doc(eventId); // Reference the task document by ID
                    await taskRef.delete(); // Delete from Firestore
                    showNotification('Task deleted successfully from Firebase!', 'success');
                } catch (error) {
                    console.error("Error removing task from Firebase: ", error);
                    showNotification('Failed to delete task from Firebase.', 'error');
                }
            } else {
